...
describe("pie chart", () => {
	...
	it("renders one arc slice per non-zero datum", async () => {
		...
		const slices = html.match(/class="xtyle-pie__slice"/g) ?? [];
	...
	it("opens a center total for the donut variant", async () => {
		...
		expect(html).toContain("xtyle-pie--donut");
		expect(html).toContain('class="xtyle-pie__center"');
	...
	it("shows a No data message for an empty or all-zero chart, with no slices", async () => {
		...
		expect(empty).toContain('class="xtyle-pie__empty"');
		expect(empty).toContain(">No data</text>");
		expect(empty).not.toContain('class="xtyle-pie__slice"');
		expect(empty).toContain('aria-label="No data"');
		const zeros = await renderFragmentLight("pie", { data: [{ label: "a", value: 0 }, { label: "b", value: 0 }] });
		expect(zeros).toContain('class="xtyle-pie__empty"');
		expect(zeros).not.toContain('class="xtyle-pie__slice"');
		const donut = await renderFragmentLight("pie", { data: [], variant: "donut" });
		expect(donut).toContain('class="xtyle-pie__empty"');
		expect(donut).not.toContain('class="xtyle-pie__center"');
	});
});
