...
describe("algorithm set", () => {
	...
	it("xtyle-hc holds AAA contrast on fg-0 / bg-0 (stricter than the rest)", () => {
		const r = derive(xtyleHc, { constraints: { "--bg-0": "#0f1115", "--accent": "#5b8cff" } });
		expect(contrast(r["--fg-0"]!, r["--bg-0"]!)).toBeGreaterThanOrEqual(7 - 0.01);
	});

	it("xtyle-hc picks the best-reachable true pole when bg-0 is pinned mid-gray", () => {
		for (const bg of ["#a0a0a0", "#808080", "#959595", "#717171"]) {
			const r = derive(xtyleHc, { constraints: { "--bg-0": bg } });
	...
	it("xtyle-hc clamps fg-0/link/accent-text to the achievable AAA floor on pinned bg-0", () => {
	...
	it("xtyle-loud carries punchier elevation than xtyle-quiet", () => {
		...
		const quiet = derive(xtyleQuiet, {})["--elevation-4"]!;
		const loud = derive(xtyleLoud, {})["--elevation-4"]!;
