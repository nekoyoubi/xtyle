...
describe("bar chart", () => {
	...
	it("renders a horizontal chart with category labels down the side", async () => {
		...
		expect(html).toContain("xtyle-bar--horizontal");
		expect(html).toContain('class="xtyle-bar__ylabels"');
		const rects = html.match(/class="xtyle-bar__bar"/g) ?? [];
	...
	it("renders bars as read-only image data points by default", async () => {
		...
		expect(html).not.toContain("xtyle-bar--selectable");
	});
	...
	it("shows a No data message with no axes when there is nothing to plot", async () => {
		...
		expect(html).toContain('class="xtyle-bar__empty"');
		expect(html).toContain(">No data</text>");
		expect(html).not.toContain('class="xtyle-bar__bar"');
		expect(html).not.toContain('class="xtyle-bar__grid"');
