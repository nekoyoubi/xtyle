...
import { manifest, fragmentSources } from "./fragments/timeline/source.generated.js";
...
/**
 * A vertical activity feed. The element owns the reading of the authored `<ol>` — which `<li>`s are
 * events, which one is last, and re-reading them whenever the list changes. The dot and the
 * connector rail are the component's own furniture, so the fill renders them as real nodes around
 * each event's authored content — a mod can swap the dot for a per-event icon or draw the rail as a
 * dashed line.
 */
export class XtyleTimeline extends XtyleElement {
	...
	private fragment = new FragmentHost(this.root, manifest, fragmentSources, "timeline", {
		applyIntent: () => {},
	 ...
	 * fill then would leave the parser appending the authored list *after* the rendered events, so the
	 ...
	 * the observer is watching — which is what re-reads the feed.
	 */
	...
	/** Each authored event and the nodes it contributes. Once relocated into the fill's content region
	 * an event's `<li>` is empty, so the nodes are read from the held map rather than re-read as nothing. */
	...
	/** A changed event count is a structural change the patch ops can't express (an event's dot, rail,
	 * and content region are three nodes, and the rail is dropped on the last one), so it rebuilds
	 * rather than patches. */
	private shapeSignature(): string {
		return String(this.items().length);
	}

	/** Move each event's authored content into the fill's content region. Nodes are relocated, never
	...
	private placeContent(): void {
		...
		this.items().forEach(({ nodes }, index) => {
			...
			const region = this.querySelector(`[data-slot="event-${index}"]`);
			if (!region) return;
	 ...
	 * events. Every write this element makes happens with the observer detached, so its own relocation
