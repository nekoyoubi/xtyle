...
export class XtyleNumberInput extends XtyleElement {
	...
	private get input(): HTMLInputElement | null {
		return this.root.querySelector(".xtyle-number__input");
	}

	private syncStepButtons(): void {
		const dec = this.root.querySelector(".xtyle-number__step--dec") as HTMLButtonElement | null;
		const inc = this.root.querySelector(".xtyle-number__step--inc") as HTMLButtonElement | null;
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("labelledby")) {
			console.warn(
				"xtyle-number-input: no accessible name. Provide a `label` or `labelledby` so the field is announced.",
			);
		}
	}
...
define("xtyle-number-input", XtyleNumberInput);
