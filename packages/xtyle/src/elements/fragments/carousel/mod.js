...
(() => {
  ...
  function dotsRail(b) {
    ...
    for (let i = 0; i < count(b); i++) {
      ...
      out += `<button type="button" class="${dotClass(isActive)}" part="dot" data-dot="${uid(b)}-${i}" data-index="${i}" role="tab" aria-selected="${isActive ? "true" : "false"}" aria-label="Go to slide ${i + 1}"></button>`;
    }
    return `${out}</div>`;
  }
  function showBar(b) {
  ...
  hooks.fragment.mount("carousel", (bindings, ops) => {
    const bar2 = showBar(bindings);
    ops.setAttr(trackSel(bindings), "aria-label", trackLabel(bindings));
    ops.replaceChildren(controlsSel(bindings), bar2 ? controlsHtml(bindings) : "");
    ops.setAttr(controlsSel(bindings), "hidden", bar2 ? "" : "hidden");
  });
  ...
  xript.exports.register("prevSlide", () => ({ nudge: -1 }));
  xript.exports.register("nextSlide", () => ({ nudge: 1 }));
  xript.exports.register("togglePlay", () => ({ togglePlay: true }));
  xript.exports.register("selectDot", (payload) => {
    const index = payload.dataset?.index;
    return index === void 0 ? {} : { select: index };
  });
})();
