...
(() => {
  ...
  function presets(b) {
    ...
    const chips = swatches.map(
      (s) => `<button class="xtyle-color-picker__preset" part="preset" type="button" data-color="${escapeAttr(s.entry)}" data-hex="${escapeAttr(s.hex)}" style="--cp-chip: ${escapeAttr(s.hex)}" aria-label="${escapeAttr(s.entry)}" aria-pressed="${String(s.pressed)}"${disabledAttr}></button>`
    ).join("");
  ...
  function harmonyRow(b) {
    ...
    const chips = (b.harmonyChips ?? []).map(
      (c) => `<button class="xtyle-color-picker__preset xtyle-color-picker__harmony-chip" part="harmony-chip" type="button" data-color="${escapeAttr(c.hex)}" data-hex="${escapeAttr(c.hex)}" style="--cp-chip: ${escapeAttr(c.hex)}" aria-label="${escapeAttr(c.hex)}" aria-pressed="false"${disabledAttr}></button>`
    ).join("");
  ...
  function channelRows(b) {
    ...
    const sliders = (b.channels ?? []).map(
      (c) => `<label class="xtyle-color-picker__channel" part="channel"><span class="xtyle-color-picker__channel-label" aria-hidden="true">${escapeHtml(c.label)}</span><input class="xtyle-color-picker__channel-input" part="channel-input" type="range" min="${c.min}" max="${c.max}" step="${c.step}" value="${c.value}" aria-label="${escapeAttr(c.name)}" aria-valuetext="${escapeAttr(c.formatted)}"${disabledAttr} /><span class="xtyle-color-picker__channel-value" part="channel-value" aria-hidden="true">${escapeHtml(c.formatted)}</span></label>`
    ).join("");
  ...
  function shell(b) {
    ...
    return `<button class="xtyle-color-picker__trigger" part="trigger" type="button" popovertarget="${escapeAttr(popoverId)}" aria-haspopup="dialog" aria-expanded="false" aria-label="Open color picker"${disabledAttr}>${TRIGGER_CARET}</button><div class="xtyle-color-picker__popover" part="popover" id="${escapeAttr(popoverId)}" popover>${body(b)}</div>`;
  }
  function inner(b) {
