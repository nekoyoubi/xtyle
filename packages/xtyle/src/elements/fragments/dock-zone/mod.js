...
(() => {
  ...
  function controls(panel) {
    ...
    for (const action of panel.actions ?? []) {
      ...
      html += `<button type="button" class="xtyle-dock-zone__action" part="action" data-panel-action data-owner-panel="${owner}" data-action-id="${escapeAttr(action.id)}" aria-label="${label}" title="${label}">${escapeHtml(action.icon ?? action.label)}</button>`;
    }
    if (panel.hasMenu) {
  ...
  function tab(panel, zoneId, active) {
    ...
    return `<button type="button" class="xtyle-dock-zone__tab${isActive}" part="tab" role="tab" aria-selected="${active}" data-tab data-panel-id="${escapeAttr(panel.id)}" data-zone-id="${escapeAttr(zoneId)}" data-index="${panel.index}">${escapeHtml(panel.title)}${badge(panel)}</button>`;
  }
  function tabsLeaf(node2) {
    ...
    return `<div class="xtyle-dock-zone__leaf" part="zone" data-zone-id="${escapeAttr(zoneId)}" style="flex:${node2.flex ?? 1}"><div class="xtyle-dock-zone__head" part="head"><div class="xtyle-dock-zone__tabs" part="tabs">${strip}</div>${activePanel ? controls(activePanel) : ""}</div><div class="xtyle-dock-zone__body" part="body" data-body-for="${escapeAttr(zoneId)}"></div></div>`;
  }
  function stackLeaf(node2) {
    ...
    const sections = (node2.panels ?? []).map((panel) => {
      ...
      return `<div class="xtyle-dock-zone__section${collapsed ? " is-collapsed" : ""}" part="section"><div class="xtyle-dock-zone__section-head" part="section-head"><button type="button" class="xtyle-dock-zone__section-toggle" part="section-toggle" aria-expanded="${!collapsed}" data-section-toggle data-panel-id="${pid}" data-zone-id="${escapeAttr(zoneId)}" data-index="${panel.index}"><span class="xtyle-dock-zone__chevron" part="chevron" aria-hidden="true"></span><span class="xtyle-dock-zone__section-title" part="section-title">${escapeHtml(panel.title)}</span>${badge(panel)}</button>${controls(panel)}</div><div class="xtyle-dock-zone__section-body" part="section-body" data-section-body-for="${pid}"${collapsed ? " hidden" : ""}></div></div>`;
    }).join("");
    return `<div class="xtyle-dock-zone__leaf xtyle-dock-zone__leaf--stack" part="zone" data-zone-id="${escapeAttr(zoneId)}" style="flex:${node2.flex ?? 1}">${sections}</div>`;
  }
  function node(n) {
    if (n.kind === "split") {
  ...
  function floatControl(panelId, verb, label, glyph) {
    ...
    return `<button type="button" class="xtyle-dock-zone__action" part="float-action" data-float-control ${verb} data-owner-panel="${escapeAttr(panelId)}" aria-label="${name}" title="${name}">${glyph}</button>`;
  }
  function floats(list) {
    if (list.length === 0) return "";
    const windows = list.map((f) => {
      const pid = escapeAttr(f.panelId);
      return `<div class="xtyle-dock-zone__float" part="float" data-float-id="${pid}" style="left:${f.x}px;top:${f.y}px;width:${f.w}px;height:${f.h}px"><div class="xtyle-dock-zone__float-head" part="float-head" data-float-head><span class="xtyle-dock-zone__float-title" part="float-title">${escapeHtml(f.title)}</span>` + floatControl(f.panelId, "data-float-dock", `Dock ${f.title}`, DOCK_GLYPH) + floatControl(f.panelId, "data-panel-close", `Close ${f.title}`, CLOSE_GLYPH) + `</div><div class="xtyle-dock-zone__float-body" part="float-body" data-float-body-for="${pid}"></div><div class="xtyle-dock-zone__float-resize" part="float-resize" data-float-resize data-owner-panel="${pid}" aria-hidden="true"></div></div>`;
    }).join("");
