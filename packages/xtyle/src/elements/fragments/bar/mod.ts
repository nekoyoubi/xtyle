...
function barClass(b: BarBindings): string {
	...
	return ["xtyle-bar", horizontal && "xtyle-bar--horizontal", b.stacked && "xtyle-bar--stacked", b.selectable && "xtyle-bar--selectable"]
		.filter(Boolean)
		.join(" ");
}
...
function verticalPlot(b: BarBindings, series: { name: string; values: number[] }[], colors: string[], height: number): string {
	...
	const baseline = `<line class="xtyle-bar__axis" x1="${plotX0}" y1="${plotY1}" x2="${plotX1}" y2="${plotY1}"></line>`;
	return (
		`<g class="xtyle-bar__grid-layer">${grid}</g>${baseline}` +
		`<g class="xtyle-bar__bars">${rects.join("")}</g>` +
		`<g class="xtyle-bar__values">${valueLabels.join("")}</g>` +
		`<g class="xtyle-bar__xlabels">${xLabels.join("")}</g>`
	);
}
...
function horizontalPlot(b: BarBindings, series: { name: string; values: number[] }[], colors: string[], height: number): string {
	...
	const baseline = `<line class="xtyle-bar__axis" x1="${plotX0}" y1="${plotY0}" x2="${plotX0}" y2="${plotY1}"></line>`;
	return (
		`<g class="xtyle-bar__grid-layer">${grid}</g>${baseline}` +
		`<g class="xtyle-bar__bars">${rects.join("")}</g>` +
		`<g class="xtyle-bar__values">${valueLabels.join("")}</g>` +
		`<g class="xtyle-bar__ylabels">${catLabels.join("")}</g>`
	);
}
