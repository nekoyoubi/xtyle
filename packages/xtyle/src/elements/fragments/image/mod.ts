...
function imageHtml(b: ImageBindings): string {
	...
	const img = src
		? `<img class="xtyle-image__img" part="image" src="${src}" alt="${alt}" loading="${loading}" decoding="async" />`
		: "";
	...
	const caption = b.caption
		? `<figcaption class="xtyle-image__caption" part="caption">${esc(b.caption)}</figcaption>`
		: "";
	return `<figure class="${imageClass(b)}" part="figure">${frame}${caption}</figure>`;
}

function render(bindings: ImageBindings, ops: OpsBuilder): void {
