...
export interface PieDatum {
	label: string;
	value: number;
	/** For the `statuses` palette, the semantic outcome this slice represents. Colors it by meaning
	 * regardless of which categories are present, so a filtered-out zero-value slice never shifts the
	 * mapping. Ignored by every other palette. */
	tone?: StatusTone;
}

export type PieScheme = Palette | string[];
