...
<xtyle-pagination page="3" total="20" href="/blog?page={page}"></xtyle-pagination>

<!-- button mode: listen for page-change -->
<xtyle-pagination page="3" total="20" tone="neutral"></xtyle-pagination>

<script>
	document.querySelector("xtyle-pagination:not([href])")
		.addEventListener("page-change", (e) => console.log(e.detail.page));
</script>`;

const svelteExample = `<script lang="ts">
	import { Pagination } from "@xtyle/svelte";
	let page = $state(3);
</script>
...
export const paginationManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xtyle-pagination__page",
		...
		},
		{
			...
			selector: ".xtyle-pagination__page--current",
	...
	variants: [
		{
			...
			className: "xtyle-pagination--accent",
			tokens: ["--accent", "--accent-fg"],
		},
		{
			name: "neutral",
			description: "Current-page pill in neutral ink for a quieter control.",
			className: "xtyle-pagination--neutral",
			tokens: ["--neutral", "--neutral-fg"],
		},
	],
	sizes: [
		{ name: "sm", description: "Compact.", className: "xtyle-pagination--sm" },
		{ name: "md", description: "Default.", className: "xtyle-pagination", isDefault: true },
		{ name: "lg", description: "Large.", className: "xtyle-pagination--lg" },
	],
	states: [
		{
			name: "hover",
			description: "Pointer over a page or control; a soft wash fills behind it.",
			selector: ".xtyle-pagination__page:hover",
			tokens: ["--bg-2"],
		},
		{
			name: "focus-visible",
			description: "Keyboard focus: a token-colored ring, plus a transparent outline promoted to a real one in forced-colors mode.",
			selector: ".xtyle-pagination__page:focus-visible",
		...
		},
		{
			...
			selector: '.xtyle-pagination__control[aria-disabled="true"]',
			tokens: ["--fg-3"],
		},
	],
	slots: [],
