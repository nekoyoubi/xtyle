...
const htmlExample = `<xtyle-code lang="ts">const greet = (name: string) =&gt; \`hi \${name}\`;</xtyle-code>

<xtyle-code lang="rust" preload>fn main() { println!("hello"); }</xtyle-code>`;
...
const htmlWrapExample = `<xtyle-code lang="ts" wrap>const message = "a long line that soft-wraps in a narrow column instead of scrolling sideways";</xtyle-code>`;
...
const htmlLineNumbersExample = `<xtyle-code lang="ts" line-numbers>function add(a: number, b: number) {
	return a + b;
}

const sum = add(2, 3);</xtyle-code>`;
...
const htmlHighlightExample = `<xtyle-code lang="ts" line-numbers highlight="2,4">function total(items) {
	let sum = 0;
...
const htmlCaptionExample = `<xtyle-code lang="ts" caption="theme.ts">export const accent = "#5b8cff";</xtyle-code>`;
...
export const codeManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xtyle-code-line[data-line-highlight]",
	...
	composition: [
		...
		"On a code-heavy page, call `XtyleCode.warm([\"ts\", \"rust\"])` once at startup, or mark individual blocks `preload`, to load grammars before first paint.",
		"For a language Prism does not ship, register it with `XtyleCode.registerLanguage(name, grammar)` and use that name as `lang`.",
