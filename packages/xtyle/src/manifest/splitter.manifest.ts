...
const htmlExample = `<div class="layout" style="display:grid;grid-template-columns:var(--rail,16rem) auto minmax(0,1fr);height:20rem">
	...
	<xtyle-splitter var="--rail" value="256" min="160" max="480" step="8" label="Resize rail"></xtyle-splitter>
	<main class="panel">Content</main>
</div>

<script>
	document.querySelector("xtyle-splitter")
		.addEventListener("resize", (e) => console.log(e.detail.value));
</script>`;

const svelteExample = `<script lang="ts">
	import { Splitter } from "@xtyle/svelte";
	let rail = $state(256);
</script>
...
export const splitterManifest: ComponentManifest = {
	...
	variants: [],
	sizes: [
		{ name: "sm", description: "A slim divider for dense layouts.", className: "xtyle-splitter--sm" },
		{ name: "md", description: "The default divider footprint.", className: "xtyle-splitter", isDefault: true },
		{ name: "lg", description: "A wide divider that's easy to grab.", className: "xtyle-splitter--lg" },
	],
	states: [
		{
			name: "keyboard-focus",
