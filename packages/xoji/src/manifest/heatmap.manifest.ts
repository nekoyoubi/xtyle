...
const htmlExample = `<xoji-heatmap
	rows='["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]'
	scale
	label="Activity by day and hour"></xoji-heatmap>
<script>
	const grid = document.querySelector("xoji-heatmap");
	grid.cols = Array.from({ length: 24 }, (_, h) => String(h));
	grid.values = weekByHour; // number[7][24]
</script>`;

const svelteExample = `<script lang="ts">
	import { Heatmap } from "@xoji/svelte";
...
const astroExample = `---
import { Heatmap } from "@xoji/astro";
const rows = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
const cols = Array.from({ length: 24 }, (_, h) => String(h));
const values = weekByHour; // number[7][24]
---
...
const schemeHtmlExample = `<xoji-heatmap
	scheme="thermal"
	show-values
	label="Load by host and resource"></xoji-heatmap>
<script>
	const grid = document.querySelector("xoji-heatmap");
...
const actionableHtmlExample = `<xoji-heatmap
	scheme="accent"
	selectable
	label="Runs per hour"></xoji-heatmap>
<script>
	const grid = document.querySelector("xoji-heatmap");
...
<script>
	document.querySelector("xoji-heatmap").addEventListener("select", (e) => {
		openRuns(e.detail.row, e.detail.col);
	});
</script>`;

const glowHtmlExample = `<xoji-heatmap
	scheme="accent"
	glow-label="runtime"
	label="Runs (fill) and total runtime (glow) by day and bucket"></xoji-heatmap>
<script>
	const grid = document.querySelector("xoji-heatmap");
	grid.rows = ["Mon", "Tue", "Wed"];
	grid.cols = ["AM", "PM", "Night"];
...
const nowHtmlExample = `<xoji-heatmap
	rows='["Mon","Tue","Wed"]'
	current-pulse
	label="Runs per bucket, this week"></xoji-heatmap>
<script>
	const grid = document.querySelector("xoji-heatmap");
	grid.cols = ["AM", "PM", "Night"];
	grid.values = [[3, 8, 1], [5, 2, 0], [0, 6, 4]];
...
export const heatmapManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xoji-heatmap__cell--current",
		...
		},
		{
			...
			selector: ".xoji-heatmap__tooltip",
	...
	props: [
		...
		},
		{
			...
			default: "false",
			description:
				"Makes cells actionable: each cell becomes a `role=\"button\"` with a pointer cursor, and clicking one (or pressing Enter/Space while it's focused) fires a `select` `CustomEvent` whose `detail` carries `{ row, col, value, rowIndex, colIndex }`. `@xoji/svelte` surfaces it as an `onselect` callback. Leave it off for a read-only grid.",
			bindings: ["html", "svelte", "astro"],
		},
		{
			name: "label",
	...
	states: [
		{
			...
			selector: ".xoji-heatmap--hovering .xoji-heatmap__cell.is-active",
		...
		},
		{
			...
			selector: ".xoji-heatmap__cell:focus-visible",
