...
const chromeExample = `<xoji-dock-zone style="height: 320px;">
	<section
...
<xoji-dock-zone mode="stack" style="height: 320px; max-width: 280px;">
...
<script type="module">
	import "@xoji/core/elements";
	document.querySelector("xoji-dock-zone").layout = {
...
export const dockZoneManifest: ComponentManifest = {
	...
	description:
		"Dock Zone is a movable-panel workspace, the editor-style chrome an app builds its layout from. Its direct children are the panels: any element with a `data-panel-id` and a `data-title` (or `title`) for its tab. The zone reads them, arranges them into a layout of tabbed zones, and renders the tab strips and splits around them. Dragging a tab re-docks its panel onto another zone, joining it as a tab when dropped over the center or splitting the zone when dropped against an edge. Dragging a tab out past every zone tears it into a floating window that moves, resizes, and docks back on the same layout. Every rearrangement dispatches a `layout-change` event carrying the serializable layout, and setting the `layout` property restores a saved one, so a workspace persists across reloads. A panel carries its own header chrome, declared on the panel child: `data-closable` for a built-in close (a cancelable `panel-close`, then removal and a fresh `layout-change`), `data-actions` for direct header buttons, and `data-menu` for a kebab overflow `<xoji-menu>`. A header button or a menu row both fire `panel-action`, and a `data-badge` puts trailing status text (a count) on the panel's own tab. A leaf renders in one of two modes: `tabs` (the default, one active panel behind a tab strip) or `stack` (every panel a collapsible section, the tool-rail shape); set `mode` for the whole workspace or per leaf in the tree. The layout physics are xoji's own headless engine (`resolveDrop` for the drop geometry, `dockPanel` for the tree), the same primitives a consumer can drive directly from `@xoji/core/elements`. The Svelte binding surfaces `layout` as a prop and reports rearrangement through `onLayoutChange`, close through `onPanelClose`, and header controls through `onPanelAction`; the Astro binding renders the panels and upgrades the workspace on the client.",
	...
	anatomy: [
		...
		{
			name: "tab",
			description: "A draggable tab; the active one carries an accent underline. A `data-badge` renders trailing status text (`.xoji-dock-zone__badge`, muted) after the title.",
		...
		},
		{
			...
			selector: ".xoji-dock-zone__actions",
		...
		},
		{
			...
			selector: ".xoji-dock-zone__section",
	...
	props: [
		...
		},
		{
			...
			description: "A kebab (⋮) overflow menu for the panel, rendered as an `<xoji-menu>` (so it carries headings, hints, and `intent: \"danger\"`). Selecting a row fires `panel-action` with the row's `value` as the `actionId`.",
			bindings: ["html", "svelte", "astro"],
		},
		{
			name: "closePanel",
	...
	composition: [
		...
		"Give a panel header controls without leaving the layout: `data-closable` for a built-in close, `data-actions` for direct buttons, and `data-menu` for a kebab `<xoji-menu>`. The chrome always reflects the active panel.",
	...
	a11y: [
		...
		"Header controls are real `<button>`s with an accessible name (`data-actions` uses its `label`, close reads `Close <title>`); the kebab menu is a full `<xoji-menu>` with keyboard navigation.",
