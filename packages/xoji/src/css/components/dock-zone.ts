...
.xoji-dock-zone__action:hover { color: var(--fg-0); background: var(--state-hover); }
.xoji-dock-zone__action:focus-visible {
	outline: var(--border-normal) solid transparent;
	box-shadow: 0 0 0 var(--border-normal) var(--ring);
}
.xoji-dock-zone__close:hover { color: var(--danger); }
.xoji-dock-zone__menu {
	display: inline-flex;
	align-items: center;
	line-height: 0;
}
.xoji-dock-zone__menu::part(trigger) {
	position: relative;
	width: var(--space-6);
	min-width: var(--space-6);
	height: var(--space-6);
...
.xoji-dock-zone__menu::part(trigger)::after {
	content: "\\22EE";
	position: absolute;
	inset: 0;
	display: grid;
...
.xoji-dock-zone__menu::part(trigger):hover { background: var(--state-hover); }
.xoji-dock-zone__menu::part(trigger):hover::after { color: var(--fg-0); }
.xoji-dock-zone__tab {
	appearance: none;
	border: none;
	background: transparent;
...
.xoji-dock-zone__leaf--stack {
	overflow-y: auto;
	overflow-x: hidden;
}
.xoji-dock-zone__section {
	display: flex;
	flex-direction: column;
	min-height: 0;
	border-bottom: var(--border-thin) solid var(--line);
}
.xoji-dock-zone__section.is-collapsed { flex: none; }
.xoji-dock-zone__section-head {
	display: flex;
	align-items: stretch;
	background: var(--bg-2);
}
.xoji-dock-zone__section-toggle {
	flex: 1;
	min-width: 0;
	appearance: none;
	border: none;
...
.xoji-dock-zone__section-toggle:hover { color: var(--fg-0); background: var(--state-hover); }
.xoji-dock-zone__section-toggle:focus-visible {
	outline: var(--border-normal) solid transparent;
	box-shadow: inset 0 0 0 var(--border-normal) var(--ring);
}
.xoji-dock-zone__chevron {
	flex: none;
	width: 0;
	height: 0;
...
.xoji-dock-zone__section:not(.is-collapsed) .xoji-dock-zone__chevron { transform: rotate(90deg); }
.xoji-dock-zone__section-title {
	overflow: hidden;
	text-overflow: ellipsis;
	white-space: nowrap;
}
.xoji-dock-zone__section-body {
	min-height: 0;
	overflow: auto;
	padding: var(--space-3);
	color: var(--fg-1);
...
.xoji-dock-zone__floats {
	position: absolute;
	inset: 0;
	z-index: 3;
	pointer-events: none;
}
.xoji-dock-zone__float {
	position: absolute;
	display: flex;
	flex-direction: column;
	min-width: 10rem;
...
.xoji-dock-zone__float.is-dragging {
	opacity: 0.55;
}
.xoji-dock-zone__float-head {
	display: flex;
	align-items: center;
	gap: var(--space-1);
...
.xoji-dock-zone__float-head:active { cursor: grabbing; }
.xoji-dock-zone__float-title {
	flex: 1;
	min-width: 0;
	overflow: hidden;
	text-overflow: ellipsis;
...
.xoji-dock-zone__float-resize:hover { opacity: 0.85; }
`.trim();
