...
/** The `xoji-icon` root class for a glyph or mark, shared by the element and the Astro binding so
...
export function iconClass(opts: { size?: string; tone?: string | null; spin?: boolean; extra?: string }): string {
	return [
		"xoji-icon",
		opts.size && opts.size !== "md" && `xoji-icon--${opts.size}`,
		opts.tone && `xoji-icon--${opts.tone}`,
		opts.spin && "xoji-icon--spin",
		opts.extra,
	]
		.filter(Boolean)
		.join(" ");
