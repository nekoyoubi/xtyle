...
function pieClass(b: PieBindings): string {
	return ["xoji-pie", b.variant === "donut" && "xoji-pie--donut"].filter(Boolean).join(" ");
}
...
function legendHtml(data: { label: string }[], colors: string[]): string {
	const items = data
		.map(
			(d, i) =>
				`<span class="xoji-pie__legend-item" part="legend-item"><span class="xoji-pie__legend-dot" style="background:${esc(colors[i] ?? "currentColor")}"></span>${esc(d.label)}</span>`,
		)
		.join("");
	return `<div class="xoji-pie__legend" part="legend">${items}</div>`;
}
...
function tableHtml(data: { label: string; value: number }[], total: number, caption: string): string {
	...
	return `<table class="xoji-pie__a11y">${cap}<thead><tr><th scope="col">Slice</th><th scope="col">Value</th><th scope="col">Share</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
function pieHtml(b: PieBindings): string {
	...
	if (total > 0) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (sweep >= 359.999) {
				slices += donut
					? `<circle class="xoji-pie__slice" part="slice" cx="${CX}" cy="${CY}" r="${((R_OUT + rIn) / 2).toFixed(2)}" fill="none" stroke="${esc(fill)}" stroke-width="${(R_OUT - rIn).toFixed(2)}" data-i="${i}" tabindex="0" role="img" aria-label="${esc(`${d.label}: ${d.value} (${percent}%)`)}"></circle>`
					: `<circle class="xoji-pie__slice" part="slice" cx="${CX}" cy="${CY}" r="${R_OUT}" fill="${esc(fill)}" data-i="${i}" tabindex="0" role="img" aria-label="${esc(`${d.label}: ${d.value} (${percent}%)`)}"></circle>`;
			} else {
				slices += `<path class="xoji-pie__slice" part="slice" d="${slicePath(R_OUT, rIn, start, end)}" fill="${esc(fill)}" data-i="${i}" tabindex="0" role="img" aria-label="${esc(`${d.label}: ${d.value} (${percent}%)`)}"></path>`;
			}
			if (b.showValues && percent >= 5) {
				const mid = polar((R_OUT + rIn) / 2 || R_OUT * 0.6, (start + end) / 2);
				labels += `<text class="xoji-pie__value" x="${mid.x.toFixed(2)}" y="${mid.y.toFixed(2)}" text-anchor="middle" dy="0.32em">${percent}%</text>`;
			}
		}
	}
	...
	const center =
		donut && total > 0
			? `<text class="xoji-pie__center" x="${CX}" y="${CY}" text-anchor="middle" dy="0.32em">${esc(formatTotal(total))}</text>`
			: "";
	const a11y = label
		? ` aria-label="${esc(label)}"`
		: ` aria-label="${total > 0 ? `Pie chart of ${data.length} slices` : "No data"}"`;
	const empty =
		total > 0 ? "" : `<text class="xoji-pie__empty" x="${CX}" y="${CY}" text-anchor="middle" dy="0.32em">No data</text>`;

	const svg =
		`<svg class="xoji-pie__svg" viewBox="0 0 ${SZ} ${SZ}" role="img"${a11y}>` +
		`<g class="xoji-pie__slices">${slices}</g><g class="xoji-pie__labels">${labels}</g>${empty}${center}` +
		`</svg>`;
