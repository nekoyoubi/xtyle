"use strict";
(() => {
  // packages/xoji/src/elements/fragments/pie/mod.ts
  var AMP = /&/g;
  var LT = /</g;
  var GT = />/g;
  var QUOT = /"/g;
  ...
  var R_IN_RATIO = 0.58;
  function dataOf(b) {
    return (b.data ?? []).map((d) => ({ label: d.label ?? "", value: Math.max(0, Number.isFinite(d.value) ? d.value : 0) })).filter((d) => d.value > 0);
  }
  function polar(r, angle) {
    const a = (angle - 90) * Math.PI / 180;
    return { x: CX + r * Math.cos(a), y: CY + r * Math.sin(a) };
  }
  function slicePath(rOut, rIn, start, end) {
  ...
  function legendHtml(data, colors) {
    const items = data.map(
      (d, i) => `<span class="xoji-pie__legend-item" part="legend-item"><span class="xoji-pie__legend-dot" style="background:${esc(colors[i] ?? "currentColor")}"></span>${esc(d.label)}</span>`
    ).join("");
  ...
  function pieHtml(b) {
    ...
    if (total > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (sweep >= 359.999) {
          slices += donut ? `<circle class="xoji-pie__slice" part="slice" cx="${CX}" cy="${CY}" r="${((R_OUT + rIn) / 2).toFixed(2)}" fill="none" stroke="${esc(fill)}" stroke-width="${(R_OUT - rIn).toFixed(2)}" data-i="${i}" tabindex="0" role="img" aria-label="${esc(`${d.label}: ${d.value} (${percent}%)`)}"></circle>` : `<circle class="xoji-pie__slice" part="slice" cx="${CX}" cy="${CY}" r="${R_OUT}" fill="${esc(fill)}" data-i="${i}" tabindex="0" role="img" aria-label="${esc(`${d.label}: ${d.value} (${percent}%)`)}"></circle>`;
        } else {
