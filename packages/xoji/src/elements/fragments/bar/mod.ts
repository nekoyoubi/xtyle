...
/** Round a raw maximum up to a clean axis ceiling (1, 2, 2.5, or 5 times a power of ten). */
...
function seriesOf(b: BarBindings): { name: string; values: number[] }[] {
	return (b.series ?? []).map((s) => ({ name: s.name ?? "", values: Array.isArray(s.values) ? s.values : [] }));
}

/** A bar's fill: by category (each category its own hue, for a single-series chart) or by series. */
function colorAt(colors: string[], colorBy: string | undefined, si: number, ci: number): string {
	return (colorBy === "category" ? colors[ci] : colors[si]) ?? "currentColor";
}

function barClass(b: BarBindings): string {
	const horizontal = b.orientation === "horizontal";
	return ["xoji-bar", horizontal && "xoji-bar--horizontal", b.stacked && "xoji-bar--stacked", b.selectable && "xoji-bar--selectable"]
		.filter(Boolean)
		.join(" ");
}
...
function legendHtml(series: { name: string }[], colors: string[]): string {
	const items = series
		.map(
			(s, i) =>
				`<span class="xoji-bar__legend-item" part="legend-item"><span class="xoji-bar__legend-dot" style="background:${esc(colors[i] ?? "currentColor")}"></span>${esc(s.name)}</span>`,
		)
		.join("");
...
function verticalPlot(b: BarBindings, series: { name: string; values: number[] }[], colors: string[], height: number): string {
	...
	const rawMax = stacked
		? Math.max(0, ...categories.map((_, ci) => series.reduce((sum, s) => sum + Math.max(0, s.values[ci] ?? 0), 0)))
		: Math.max(0, ...series.flatMap((s) => s.values.map((v) => Math.max(0, v ?? 0))));
	const max = niceMax(rawMax);
	const yOf = (v: number): number => plotY1 - (Math.max(0, v) / max) * plotH;
	...
	return (
		`<g class="xoji-bar__grid-layer">${grid}</g>${baseline}` +
		`<g class="xoji-bar__bars">${rects.join("")}</g>` +
		`<g class="xoji-bar__values">${valueLabels.join("")}</g>` +
		`<g class="xoji-bar__xlabels">${xLabels.join("")}</g>`
	);
}

function horizontalPlot(b: BarBindings, series: { name: string; values: number[] }[], colors: string[], height: number): string {
	const categories = b.categories ?? [];
	const stacked = !!b.stacked;
	const selectable = !!b.selectable;
	...
	const rawMax = stacked
		...
		: Math.max(0, ...series.flatMap((s) => s.values.map((v) => Math.max(0, v ?? 0))));
	const max = niceMax(rawMax);
	const xOf = (v: number): number => plotX0 + (Math.max(0, v) / max) * plotW;
	...
	return (
		...
		`<g class="xoji-bar__bars">${rects.join("")}</g>` +
		`<g class="xoji-bar__values">${valueLabels.join("")}</g>` +
		`<g class="xoji-bar__ylabels">${catLabels.join("")}</g>`
	);
}

function barHtml(b: BarBindings): string {
	...
	const plot = horizontal
		? horizontalPlot(b, series, colors, height)
		: verticalPlot(b, series, colors, height);

	const svg =
		`<svg class="xoji-bar__svg" viewBox="0 0 ${IW} ${height}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">${plot}</svg>`;
