...
interface PanelMeta {
	...
	actions: PanelAction[];
	/** A kebab overflow menu (an `<xoji-menu>`) for the operations that don't earn a dedicated button. */
	menu: MenuItem[] | null;
}
 ...
 * buttons, and `data-menu` (a JSON menu-item array) renders a kebab overflow `<xoji-menu>`. Both a
...
export class XojiDockZone extends HTMLElement {
	...
	private parsePanelActions(child: HTMLElement): PanelAction[] {
		...
		try {
			...
			console.warn("xoji-dock-zone: malformed `data-actions` on a panel; expected a JSON array of { id, label, icon? }.", raw);
			return [];
		}
	}
	...
	private parsePanelMenu(child: HTMLElement): MenuItem[] | null {
		...
		try {
			...
			console.warn("xoji-dock-zone: malformed `data-menu` on a panel; expected a JSON array of menu items.", raw);
			return null;
		}
	}
	...
	private renderNode(node: DockNode): HTMLElement {
		...
		head.className = "xoji-dock-zone__head";
	...
	private renderStackLeaf(node: DockLeaf): HTMLElement {
		const zone = document.createElement("div");
		zone.className = "xoji-dock-zone__leaf xoji-dock-zone__leaf--stack";
		zone.dataset.dockChrome = "";
		zone.dataset.zoneId = node.id;
		zone.style.flex = "1";
		...
		node.panels.forEach((pid, i) => {
			...
			section.className = "xoji-dock-zone__section" + (isCollapsed ? " is-collapsed" : "");
			const head = document.createElement("div");
			head.className = "xoji-dock-zone__section-head";
			const toggle = document.createElement("button");
			toggle.type = "button";
			toggle.className = "xoji-dock-zone__section-toggle";
			toggle.dataset.panelId = pid;
			toggle.setAttribute("aria-expanded", String(!isCollapsed));
			const chevron = document.createElement("span");
			chevron.className = "xoji-dock-zone__chevron";
			chevron.setAttribute("aria-hidden", "true");
			const title = document.createElement("span");
			title.className = "xoji-dock-zone__section-title";
			...
			if (actions) head.appendChild(actions);
			const body = document.createElement("div");
			body.className = "xoji-dock-zone__section-body";
	...
	private badgeEl(meta: PanelMeta | undefined): HTMLElement | null {
		...
		badge.className = "xoji-dock-zone__badge";
	...
	private renderActions(meta: PanelMeta): HTMLElement | null {
		...
		area.className = "xoji-dock-zone__actions";
		area.dataset.dockChrome = "";
		for (const action of meta.actions) {
			const btn = document.createElement("button");
			btn.type = "button";
			btn.className = "xoji-dock-zone__action";
		...
		if (meta.menu) {
			const menu = document.createElement("xoji-menu") as HTMLElement & { items: MenuItem[] };
			menu.className = "xoji-dock-zone__menu";
		...
		if (meta.closable) {
			...
			close.type = "button";
			close.className = "xoji-dock-zone__action xoji-dock-zone__close";
	...
	private renderFloats(): void {
		...
		if (!this.floatLayer) {
			...
			this.floatLayer.className = "xoji-dock-zone__floats";
		...
		for (const f of this._floating) {
			...
			win.className = "xoji-dock-zone__float";
			...
			const head = document.createElement("div");
			head.className = "xoji-dock-zone__float-head";
			const title = document.createElement("span");
			title.className = "xoji-dock-zone__float-title";
			...
			const body = document.createElement("div");
			body.className = "xoji-dock-zone__float-body";
			const resize = document.createElement("div");
			resize.className = "xoji-dock-zone__float-resize";
	...
	private onFloatResizedown(event: PointerEvent, panelId: string): void {
		...
		const host = this.getBoundingClientRect();
		const maxW = Math.max(XojiDockZone.FLOAT_MIN_W, host.width - start.x);
		const maxH = Math.max(XojiDockZone.FLOAT_MIN_H, host.height - start.y);
		let next: FloatRect = { x: start.x, y: start.y, w: start.w, h: start.h };
		this.trackPointerDrag(
			(e) => {
				const w = Math.max(XojiDockZone.FLOAT_MIN_W, Math.min(maxW, Math.round(start.w + (e.clientX - event.clientX))));
				const h = Math.max(XojiDockZone.FLOAT_MIN_H, Math.min(maxH, Math.round(start.h + (e.clientY - event.clientY))));
	...
	private floatPanelAtPointer(panelId: string, event: PointerEvent): void {
		...
		const y = Math.max(0, Math.min(maxY, Math.round(event.clientY - host.top - XojiDockZone.FLOAT_TITLE_H)));
