...
export class XojiHeatmap extends XojiElement {
	...
	private wireInteraction(): void {
		const chart = this.root.querySelector<HTMLElement>(".xoji-heatmap");
		const tooltip = this.root.querySelector<HTMLElement>(".xoji-heatmap__tooltip");
		if (!chart || !tooltip) return;
		const cells = [...this.root.querySelectorAll<SVGRectElement>(".xoji-heatmap__cell")];
		const values = this.values;
		...
		const show = (cell: SVGRectElement): void => {
			...
			if (title) {
				...
				const glowPart =
					glowLabel && Number.isFinite(glowVal)
						? ` <span class="xoji-heatmap__tooltip-name">· ${escapeHtml(glowLabel)} ${glowVal}</span>`
						: "";
				tooltip.innerHTML = `<span class="xoji-heatmap__tooltip-name">${name}</span> <span class="xoji-heatmap__tooltip-value">${value}</span>${glowPart}`;
			}
			...
			chart.classList.add("xoji-heatmap--hovering");
			for (const other of cells) other.classList.toggle("is-active", other === cell);
		};
		const hide = (): void => {
			tooltip.hidden = true;
			chart.classList.remove("xoji-heatmap--hovering");
...
define("xoji-heatmap", XojiHeatmap);
