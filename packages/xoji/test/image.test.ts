...
describe("image", () => {
	...
	it("renders a figure with a framed image and a shimmer placeholder", async () => {
		...
		expect(html).toContain("xoji-image__frame");
		expect(html).toContain("xoji-image__placeholder");
		expect(html).toContain('<img class="xoji-image__img"');
	...
	it("reflects fit and radius as classes, defaulting to cover and md", async () => {
		...
		expect(contain).toContain("xoji-image--contain");
		expect(contain).toContain("xoji-image--radius-lg");

		const base = await renderFragmentLight("image", { src: "/a.jpg", alt: "A" });
		expect(base).not.toContain("xoji-image--contain");
		expect(base).not.toContain("xoji-image--radius-md");
	});
	...
	it("sizes the lightbox with a definite frame so an intrinsic-less image can't collapse it", () => {
		...
		const lightbox = imageCss.slice(imageCss.indexOf(".xoji-image__lightbox {"));
		const block = lightbox.slice(0, lightbox.indexOf("}"));
		expect(block).toContain("width: 92vw");
		expect(block).toContain("height: 92vh");
		const full = imageCss.slice(imageCss.indexOf(".xoji-image__full {"));
