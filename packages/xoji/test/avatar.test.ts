...
describe("avatar status-dot pulse", () => {
	it("a bare / true pulse breathes the status dot at the slow cadence", () => {
		expect(avatarClass({ status: "success", pulse: true })).toContain("xoji-avatar--pulse-slow");
		expect(avatarClass({ status: "success", pulse: "slow" })).toContain("xoji-avatar--pulse-slow");
	});

	it("pulse=\"fast\" breathes at the quick cadence", () => {
		expect(avatarClass({ status: "success", pulse: "fast" })).toContain("xoji-avatar--pulse-fast");
	});
	...
	it("renders the pulse class alongside the status dot in the markup", () => {
		...
		expect(html).toContain("xoji-avatar--pulse-slow");
		expect(html).toContain("xoji-avatar__status-dot");
	});
});
