...
describe("breadcrumbMarkup", () => {
	it("renders an href crumb as a link and the last crumb as the current page", () => {
		...
		expect(html).toContain('<a class="xoji-breadcrumb__link" part="item" href="/">Home</a>');
		expect(html).toContain('<span class="xoji-breadcrumb__current" part="item" aria-current="page">Now</span>');
	});
	...
	it("renders a valued crumb (no href) as a button carrying its value", () => {
		...
		expect(html).toContain('<button type="button" class="xoji-breadcrumb__link" part="item" data-value="0">root</button>');
	});

	it("prefers href over value when both are set", () => {
		const html = breadcrumbMarkup({ items: [{ label: "node", href: "/n", value: "0" }, { label: "leaf" }] });
		expect(html).toContain('<a class="xoji-breadcrumb__link" part="item" href="/n">node</a>');
	...
	it("projects consumer crumbs through a bare slot (valid list nesting, no wrapper span)", () => {
		...
		expect(html).toContain('<ol class="xoji-breadcrumb__list" part="list"><slot></slot></ol>');
		expect(html).not.toContain("xoji-slot");
	});
});
